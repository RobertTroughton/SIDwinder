//; =============================================================================
//;                  3-CHANNEL BAR ANALYSIS MODULE (+ WAVEFORM)
...
//;
//;        This variant additionally records, per bar, the WAVEFORM ("instrument
//;        type") of the voice that owns that bar, so the renderer can pick a
//;        colour set by timbre rather than by channel:
//;            0 = triangle (or no waveform)   1 = sawtooth
//;            2 = pulse                        3 = noise
//;        Classification is by priority noise > pulse > saw > triangle.
//; =============================================================================
...
//; Per-bar waveform / colour-set index (0..3), updated when a voice claims a bar.
.align NUM_FREQUENCY_BARS
barWaveCh0:             .fill NUM_FREQUENCY_BARS, 0
.align NUM_FREQUENCY_BARS
barWaveCh1:             .fill NUM_FREQUENCY_BARS, 0
.align NUM_FREQUENCY_BARS
barWaveCh2:             .fill NUM_FREQUENCY_BARS, 0
...
//; Scratch: waveform/colour-set index of the voice currently being analysed.
analysisWave:           .byte 0
...
!gotBar:
    // X = bar index. Classify the waveform (control register, offset 4) into a
    // colour-set index 0..3 -> analysisWave. Preserves X.
    ldy #4
    lda (zpRegPtr), y
    ldy #$00                        // default: triangle / none
    asl                             // bit7 (noise) -> carry
    bcc !notNoise+
    ldy #$03
    jmp !waveDone+
!notNoise:
    asl                             // bit6 (pulse) -> carry
    bcc !notPulse+
    ldy #$02
    jmp !waveDone+
!notPulse:
    asl                             // bit5 (sawtooth) -> carry
    bcc !waveDone+
    ldy #$01
!waveDone:
    sty analysisWave
    ...
    sta zpTempByte                  // save target height
