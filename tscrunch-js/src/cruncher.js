 ...
 *
 * The Cruncher builds an optimal token graph for the input and walks the
 * shortest path through it (Dijkstra) to produce the smallest crunched stream.
 */

import {
    RLE, LZ, LZ2, LIT, ZERORUN,
    findOptimalZero,
...
// Node-only stdout helper. Guarded so the library is safe to import in browsers.
const hasStdout = typeof process !== 'undefined' && process.stdout && typeof process.stdout.write === 'function';
...
class Cruncher {
     ...
     * @param {boolean} options.verbose - Show progress output (Node only)
    ...
    ocrunch(options = {}) {
        ...
        if (verbose || progressCallback) {
            ...
            if (verbose && hasStdout) process.stdout.write('\n');
        }
        ...
        // Find shortest path
        const { predecessors } = dijkstra(dijkstraGraph, 0);
