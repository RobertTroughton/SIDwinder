...
private:
    ...
    void convertCharCell(int charX, int charY, uint8_t bgColor) {
        ...
        uint8_t color1 = 0; // Will map to background if not used
        uint8_t color2 = 0; // Will map to background if not used  
        uint8_t color3 = 0; // Will map to background if not used

        // Assign actual colors we found
        if (colors.size() >= 1) color1 = colors[0];
        if (colors.size() >= 2) color2 = colors[1];
        if (colors.size() >= 3) color3 = colors[2];

        scrData[screenIndex] = (color1 << 4) | color2;
        colData[screenIndex] = color3;
        ...
        for (int y = 0; y < 8; y++) {
            ...
            for (int x = 0; x < 8; x += 2) {
                ...
                uint8_t colorIndex = 0; // Default to background (00)

                // Map pixel color to 2-bit value
                if (pixelColor == bgColor) {
                    colorIndex = 0; // Background (00)
                }
                else if (colors.size() >= 1 && pixelColor == color1) {
                    colorIndex = 1; // Upper screen nibble (01)
                }
                else if (colors.size() >= 2 && pixelColor == color2) {
                    colorIndex = 2; // Lower screen nibble (10)  
                }
                else if (colors.size() >= 3 && pixelColor == color3) {
                    colorIndex = 3; // Color memory (11)
                }
                // If no match found, default to background (colorIndex = 0)
